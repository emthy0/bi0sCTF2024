  ...
  try {
    ...
    if (author) {
      if (typeof author !== 'string') {
        return res.status(500).json({ Message: 'Internal server error author not str' });
      }
      if (author.length > 86) {
        return res.status(500).json({ Message: 'Internal server error author too long' });
      }
      if (!/^[A-Za-z0-9/."\\(){};=]+$/.test(author)) {
        return res.status(500).json({ Message: 'Internal server error author regex' + author});
      }
    ...
    if (title) {
      if (typeof title !== 'string') {
        return res.status(400).json({ Message: 'Internal server error title not str' });
      }
      if (title.length > 86) {
        return res.status(500).json({ Message: 'Internal server error title too liong' });
      }
      if (!/^[A-Za-z0-9/."\\(){};=]+$/.test(title)) {
        return res.status(500).json({ Message: 'Internal server error title regex' });
      }
      protoContents[5] = `  ${author} string author = 3 [default="user"];`;
    }
